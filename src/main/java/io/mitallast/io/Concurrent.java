...
public interface Concurrent<F extends Higher> extends Async<F> {
     ...
     * }}}
     */
    default <A> Higher<F, A> cancelable(Function1<Consumer<Either<Throwable, A>>, Higher<F, Unit>> k) {
        return defaultCancelable(k, this);
    }
     ...
     */
    static <F extends Higher, A> Higher<F, A> liftIO(IO<A> ioa, Concurrent<F> F) {
        if (ioa instanceof IO.Pure) {
            return F.pure(((IO.Pure<A>) ioa).a());
        } else if (ioa instanceof IO.RaiseError) {
            return F.raiseError(((IO.RaiseError<A>) ioa).e());
        } else if (ioa instanceof IO.Delay) {
            return F.delay(((IO.Delay<A>) ioa).thunk());
        } else {
            return F.suspend(() -> {
                var io = IORunLoop.step(ioa);
                if (io instanceof IO.Pure) {
                    ...
                    return F.cancelable(cb -> liftIO(io.unsafeRunCancelable(cb), F));
                }
            });
        }
    }
     ...
     *                 the source canceled
     */
    static <F extends Higher, A> Higher<F, A> timeoutTo(Higher<F, A> fa,
                                                        Duration duration,
                                                        Higher<F, A> fallback,
                                                        Concurrent<F> F,
                                                        Timer<F> timer) {
        var r = F.race(fa, timer.sleep(duration));
        return F.flatMap(r, e -> e.fold(
            a -> F.pure(a),
            u -> fallback
        ));
    }
     ...
     */
    static <F extends Higher, A> Higher<F, A> timeout(Higher<F, A> fa,
                                                      Duration duration,
                                                      Concurrent<F> F,
                                                      Timer<F> timer) {
        return timeoutTo(fa, duration, F.raiseError(new TimeoutException(duration.toString())), F, timer);
    }

    private static <F extends Higher, A> Higher<F, A> defaultCancelable(
        // k: (Either[Throwable, A] => Unit
        Function1<Consumer<Either<Throwable, A>>, Higher<F, Unit>> k,
        Async<F> F) {
        return F.asyncF(cb -> {
            ...
            var latch = new CompletableFuture<Unit>();
            var latchF = F.<Unit>async(cb2 -> latch.whenComplete((u, err) -> cb2.accept(Either.right(Unit.unit()))));

            // Side-effecting call; unfreezes latch in order to allow bracket to finish
            var token = k.apply(result -> {
                latch.complete(Unit.unit());
                cb.accept(result);
            });

            return F.bracketCase(F.pure(token), u -> latchF, (cancel, ec) -> {
                if (ec instanceof ExitCase.Canceled) {
                    return cancel;
                } else {
                    return F.unit();
                }
